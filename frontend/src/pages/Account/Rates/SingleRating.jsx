...
function SingleRating({ review, reviewed, setUser, setReviewed, setRatingAlert }) {
    ...
    async function handleRemove() {
        const deletedReview = await deleteReview(review.id, review.ratings.rating, review.ratings.user);
        if (deletedReview.found) {
            ...
            setUser((user) => ({
                ...user,
                ratings: user.ratings.filter((rating) => rating.product != review.id)
            }));
        }
    }
        ...
        <div className="flex flex-col">
            <div className={`w-full p-4 flex items-center gap-5`}>
                <span className="min-w-32 flex justify-center items-center cursor-pointer">
                    ...
                    >
                        <img src={review.icon.url} className="h-20 w-20" />
                    </Link>
                </span>
                <span>
                    <Link
                    ...
                    <p className="text-sm text-gray-600"><Rating allowFraction readonly={true} initialValue={review.ratings.rating} /></p>
